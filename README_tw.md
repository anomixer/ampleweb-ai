...
*   **AI Agent 對話控制面板（邊玩邊聊的 AI 副駕駛）**：位於右側控制面板的 **AI Agent 頁籤**，讓您在 AI 遊玩的**同時**與它對話。您可以隨時下指令（「往北走並拿起油燈」）、提問（「你現在看到什麼？」），或直接說「幫我玩這個遊戲」——若 AI 代理尚未啟動，送出訊息會自動啟動它，訊息會與螢幕狀態一起注入下一輪 LLM 請求。AI 透過結構化回應中的 `Reply:` 區塊回話，以聊天氣泡呈現，並附上它實際執行的按鍵指令。保持模擬器螢幕 100% 無遮擋。
