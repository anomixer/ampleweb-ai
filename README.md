...
*   **AI Agent Chat Interface (Conversational Co-Pilot)**: Embedded cleanly within the right sidebar's **AI Agent tab**, allowing you to converse with the AI agent *while it plays*. Type instructions ("go north and take the lamp"), ask questions ("what do you see?"), or just say "play this game" — sending a message auto-starts the agent if it is not running yet, and your message is injected into the next LLM turn alongside the screen state. The AI answers back through the `Reply:` section of its structured response, shown as chat bubbles together with the keystrokes it executed, keeping the emulator canvas 100% unobstructed.
*   **Game Profiles (Multi-Game Generalization)**: The agent is no longer hardwired to Zork. A **Game Profile** (name + genre + input style + play hints) drives the system prompt via `buildSystemPrompt()`. Built-in presets include Zork I, Karateka, Choplifter, Super Othello, plus **Generic Text Adventure** and **Generic Action Game** profiles for unknown titles. Genre also tunes the tick rate automatically (action 2s / strategy 5s / adventure 10s), and `inputStyle` switches between full-sentence typing and real-time key presses.
    *   *Auto-Detection from Disk Names*: When you mount a disk (local file or URL), the filename is matched against a keyword table (`detectGameProfileId`) — e.g. `karateka.dsk` selects the Karateka profile, any Infocom title selects the generic text-adventure profile with the disk name as the game name. You can always override manually in the AI tab.
