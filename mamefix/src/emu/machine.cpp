...
uint8_t running_machine::emscripten_read_ram(uint32_t addr) {
	if (!emscripten_running_machine) return 0;
	device_t *cpu = emscripten_running_machine->root_device().subdevice("maincpu");
	if (cpu) {
		device_memory_interface *memory;
		if (cpu->interface(memory)) {
			return memory->space(AS_PROGRAM).read_byte(addr);
		}
	}
	return 0;
}

uint32_t running_machine::emscripten_read_ram_bulk(uint32_t start_addr, uint32_t length, uint8_t *out_buf) {
	if (!emscripten_running_machine || !out_buf) return 0;
	device_t *cpu = emscripten_running_machine->root_device().subdevice("maincpu");
	if (cpu) {
		device_memory_interface *memory;
		if (cpu->interface(memory)) {
			auto &space = memory->space(AS_PROGRAM);
			for (uint32_t i = 0; i < length; i++) {
				out_buf[i] = space.read_byte(start_addr + i);
			}
			return length;
		}
	}
...
uint8_t running_machine::emscripten_read_machine_ram(const char* share_name, uint32_t addr) {
	if (!emscripten_running_machine || !share_name) return 0;
	auto const &shares = emscripten_running_machine->memory().shares();
	auto search = shares.find(share_name);
	if (search != shares.end()) {
		void *ptr = search->second->ptr();
		size_t bytes = search->second->bytes();
		if (ptr && addr < bytes) {
			return static_cast<uint8_t*>(ptr)[addr];
		}
	}
	return 0;
}

uint32_t running_machine::emscripten_read_machine_ram_bulk(const char* share_name, uint32_t addr, uint32_t length, uint8_t *out_buf) {
	if (!emscripten_running_machine || !share_name || !out_buf) return 0;
	auto const &shares = emscripten_running_machine->memory().shares();
	auto search = shares.find(share_name);
	if (search != shares.end()) {
		void *ptr = search->second->ptr();
		...
		if (ptr && addr < bytes) {
			...
			std::memcpy(out_buf, static_cast<uint8_t*>(ptr) + addr, to_copy);
			return to_copy;
		}
	}
	return 0;
...
#include "../devices/machine/ram.h"
#include "../mame/apple/apple2video.h"
#include "../devices/bus/a2bus/a2eauxslot.h"

uint8_t running_machine::emscripten_read_main_ram(uint32_t addr) {
	if (!emscripten_running_machine) return 0;
	a2_video_device *video = dynamic_cast<a2_video_device *>(emscripten_running_machine->root_device().subdevice("a2video"));
	static int diag_count = 0;
	if (diag_count < 5) {
		++diag_count;
		// Scan all devices for aux card via device_enumerator
		device_a2eauxslot_card_interface *found_card = nullptr;
		const char *found_tag = "(none)";
		for (device_t &dev : device_enumerator(emscripten_running_machine->root_device())) {
			device_a2eauxslot_card_interface *c = dynamic_cast<device_a2eauxslot_card_interface *>(&dev);
			if (c && c->get_vram_ptr()) {
				found_card = c;
				found_tag = dev.tag();
				break;
		...
		printf("[WASM DIAG #%d] a2video=%s main=%p a2vid_aux=%p | scan_card_tag=%s vram=%p\n",
			diag_count,
			video ? "FOUND" : "NOT_FOUND",
			video ? (void*)video->get_ram_ptr() : nullptr,
			video ? (void*)video->get_aux_ptr() : nullptr,
			found_tag,
			found_card ? (void*)found_card->get_vram_ptr() : nullptr);
	}
	if (video && video->get_ram_ptr()) {
		return video->get_ram_ptr()[addr % 65536];
	}
	ram_device *ram = dynamic_cast<ram_device *>(emscripten_running_machine->root_device().subdevice("ram"));
	if (ram && ram->pointer()) {
		return ram->pointer()[addr % ram->size()];
	}
	return 0;
}

uint32_t running_machine::emscripten_read_main_ram_bulk(uint32_t start_addr, uint32_t length, uint8_t *out_buf) {
	if (!emscripten_running_machine || !out_buf) return 0;
	a2_video_device *video = dynamic_cast<a2_video_device *>(emscripten_running_machine->root_device().subdevice("a2video"));
	if (video && video->get_ram_ptr()) {
		uint8_t *ram_ptr = video->get_ram_ptr();
		for (uint32_t i = 0; i < length; i++) {
			out_buf[i] = ram_ptr[(start_addr + i) % 65536];
		}
		return length;
	}
	ram_device *ram = dynamic_cast<ram_device *>(emscripten_running_machine->root_device().subdevice("ram"));
	if (ram && ram->pointer()) {
		uint8_t *ram_ptr = ram->pointer();
		uint32_t size = ram->size();
		for (uint32_t i = 0; i < length; i++) {
			out_buf[i] = ram_ptr[(start_addr + i) % size];
		}
		return length;
	}
	return 0;
...
// Helper: find aux RAM pointer for Apple IIe 80-col mode.
// Uses device_enumerator to scan ALL devices - completely tag/cast independent.
static uint8_t *find_aux_ram_ptr(running_machine *machine) {
	if (!machine) return nullptr;

	// Priority 1: a2_video_device already has the ptr set by apple2e machine_start
	a2_video_device *video = dynamic_cast<a2_video_device *>(
		machine->root_device().subdevice("a2video"));
	if (video && video->get_aux_ptr()) {
		return video->get_aux_ptr();
	}

	// Priority 2: Scan ALL devices in tree for device_a2eauxslot_card_interface.
	// This finds the 80-col card regardless of its tag name or position in hierarchy.
	for (device_t &dev : device_enumerator(machine->root_device())) {
		device_a2eauxslot_card_interface *card =
			dynamic_cast<device_a2eauxslot_card_interface *>(&dev);
		if (card) {
			uint8_t *ptr = card->get_vram_ptr();
			if (ptr) return ptr;
		}
	}

	// Priority 3: 128KB ram_device (Apple IIc: aux lives at offset +64KB)
	ram_device *ram = dynamic_cast<ram_device *>(
		machine->root_device().subdevice("ram"));
	if (ram && ram->pointer() && ram->size() >= 128 * 1024) {
		return ram->pointer() + 65536;
	}

	return nullptr;
}

uint8_t running_machine::emscripten_read_aux_ram(uint32_t addr) {
	uint8_t *aux_ptr = find_aux_ram_ptr(emscripten_running_machine);
	if (aux_ptr) {
		return aux_ptr[addr % 65536];
	}
	return 0;
}

uint32_t running_machine::emscripten_read_aux_ram_bulk(uint32_t start_addr, uint32_t length, uint8_t *out_buf) {
	if (!emscripten_running_machine || !out_buf) return 0;
	uint8_t *aux_ptr = find_aux_ram_ptr(emscripten_running_machine);

	// Diagnostic: print aux_ptr result on first 5 bulk reads
	static int aux_diag_count = 0;
	if (aux_diag_count < 5) {
		++aux_diag_count;
		if (aux_ptr) {
			printf("[AUX BULK DIAG #%d] find_aux_ram_ptr OK ptr=%p sample[0x400]=0x%02x\n",
				aux_diag_count, (void*)aux_ptr, (unsigned)aux_ptr[0x400]);
		} else {
			// Diagnose why it failed
			a2_video_device *video = dynamic_cast<a2_video_device *>(
				emscripten_running_machine->root_device().subdevice("a2video"));
			int found_cards = 0;
			for (device_t &dev : device_enumerator(emscripten_running_machine->root_device())) {
				device_a2eauxslot_card_interface *c = dynamic_cast<device_a2eauxslot_card_interface *>(&dev);
				if (c) {
					++found_cards;
					printf("[AUX BULK DIAG #%d] found card tag=%s vram=%p\n",
						aux_diag_count, dev.tag(), (void*)c->get_vram_ptr());
				}
			}
			printf("[AUX BULK DIAG #%d] find_aux_ram_ptr FAILED. a2video=%s aux=%p cards_found=%d\n",
				aux_diag_count,
				video ? "FOUND" : "NOT_FOUND",
				video ? (void*)video->get_aux_ptr() : nullptr,
				found_cards);
	...
	if (aux_ptr) {
		for (uint32_t i = 0; i < length; i++) {
			out_buf[i] = aux_ptr[(start_addr + i) % 65536];
		}
		return length;
	}
	return 0;
...
// Standalone C exports: return WASM linear-memory offset of main/aux RAM start.
// In Emscripten, C++ pointers ARE WASM linear-memory offsets (uint32).
// Call from JS as: M._emscripten_get_main_ram_wasm_offset() etc.
extern "C" {
	EMSCRIPTEN_KEEPALIVE uint32_t emscripten_get_main_ram_wasm_offset() {
		running_machine *machine = running_machine::emscripten_get_running_machine();
		if (!machine) return 0;
		a2_video_device *video = dynamic_cast<a2_video_device *>(
			machine->root_device().subdevice("a2video"));
		if (video && video->get_ram_ptr())
			return (uint32_t)(uintptr_t)video->get_ram_ptr();
		return 0;
	}

	EMSCRIPTEN_KEEPALIVE uint32_t emscripten_get_aux_ram_wasm_offset() {
		uint8_t *ptr = find_aux_ram_ptr(running_machine::emscripten_get_running_machine());
		return ptr ? (uint32_t)(uintptr_t)ptr : 0;
	}
}
